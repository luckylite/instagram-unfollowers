...
command_template = """curl '{url}' -H 'cookie: mid=XPbBggAEAAG0sSTlc9ocHIgdJa9N; csrftoken=H661LUAaOXR8N0rrnKlebq9INrXcX8eB; shbid=3433; shbts=1559675352.786263; ds_user_id=8528156345; sessionid=8528156345%3AvFM7xD8GtJM848%3A15; rur=PRN; urlgen="{{\"93.170.68.19\": 43656}}:1hYGjr:1vFHGUbSlJVnt3Vlg-VJMfqQBfQ"' -H 'accept-encoding: gzip, deflate, br' -H 'accept-language: ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7,uk;q=0.6' -H 'user-agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.169 Safari/537.36' -H 'accept: */*' -H 'referer: https://www.instagram.com/vlad9__kandyba/following/' -H 'authority: www.instagram.com' -H 'x-requested-with: XMLHttpRequest' -H 'x-ig-app-id: 936619743392459' --compressed > json_subscriptions/subscriptions_{index}.json"""
...
print('Start getting subscriptions.')
...
while True:
    ...
    with open(f'json_subscriptions/subscriptions_{index}.json', 'r') as f:
        data = json.load(f)

    after = data['data']['user']['edge_follow']['page_info']['end_cursor']

    all_followers = data['data']['user']['edge_follow']['count']
    followers_processed += len(data['data']['user']['edge_follow']['edges'])

    print(f'Processed {followers_processed}/{all_followers}')

    next_page = data['data']['user']['edge_follow']['page_info']['has_next_page']

    if not next_page:
        print('Finish getting subscriptions:)')
        break
